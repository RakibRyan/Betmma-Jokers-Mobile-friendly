...
do
    ...
    function Card:set_ability(center, initial, delay_sprites)
        ...
        if center and betmma_smaller_sets[center.set] then
            -- Fix for oversized sprites: dynamically scale based on 34x34 sprite resolution
            self.T.w = G.CARD_W * (34/71)
            self.T.h = G.CARD_H * (34/95)
        end
    ...
    -- make Abilities appear as 34*34 instead of card size 71*95
...
do
    function GET_PATH_COMPAT()
        ...
        local shader_path = path .. "/shaders/" .. v .. ".fs"
        
        -- 1. Try LÖVE's virtual filesystem first (Bypasses Android security blocks)
        local file = love.filesystem.read(shader_path)
        
        -- 2. Fallback to NativeFS for older PC mod loaders
        if not file and NFS then
            file = NFS.read(shader_path)
        end
        
        -- 3. Extreme fallback to prevent the 'nil' crash if the file is completely missing
        if not file then
            file = "extern number time; vec4 effect(vec4 c, Image t, vec2 tc, vec2 sc) { return Texel(t, tc) * c; }"
        end
...
SMODS.ConsumableType { -- Define Ability Consumable Type
    ...
    create_UIBox_your_collection = function(self)
        ...
        local page = G.your_collection_page or 1
        
        -- Actual dimensions for a 34x34 sprite relative to a standard 71x95 card
        local abil_w = G.CARD_W * (34/71)
        local abil_h = G.CARD_H * (34/95)

        for j = 1, #self.collection_rows do
            G.your_collection[j] = CardArea(
                G.ROOM.T.x + 0.2 * G.ROOM.T.w / 2, G.ROOM.T.h,
                (self.collection_rows[j] + 0.5) * abil_w, -- Perfect horizontal spacing for 6 small cards
                abil_h * 1.1, -- Slightly taller than the card to prevent vertical overlap
                -- Set to 'title' so the UI doesn't draw grey consumable slots
        ...
        local limit = 0
        for _, rows in ipairs(self.collection_rows) do
            limit = limit + rows
        end
        local total_pages = math.max(1, math.ceil(#consumable_pool / limit))
        page = math.min(page, total_pages)

        local start_idx = (page - 1) * limit
        local current_idx = start_idx

        for j = 1, #G.your_collection do
            for i = 1, self.collection_rows[j] do
                current_idx = current_idx + 1
                local center = consumable_pool[current_idx]
                if not center then break end
                -- Spawn the cards with the corrected smaller dimensions
                local card = Card(G.your_collection[j].T.x + G.your_collection[j].T.w / 2, G.your_collection[j].T.y,
                    abil_w, abil_h, nil, center)
        ...
        for i = 1, total_pages do
            table.insert(center_options, localize('k_page') .. ' ' .. tostring(i) .. '/' .. tostring(total_pages))
        end
...
G.FUNCS.your_collection_ability_page = function(args)
    if not args or not args.cycle_config then return end
    G.your_collection_page = args.cycle_config.current_option
    
    local consumable_pool = {}
    if G.ACTIVE_MOD_UI then
        for _, v in ipairs(G.P_CENTER_POOLS['Ability']) do
            if v.mod and G.ACTIVE_MOD_UI.id == v.mod.id then consumable_pool[#consumable_pool+1] = v end
        end
    else
        consumable_pool = G.P_CENTER_POOLS['Ability']
    end

    local collection_rows = { 6, 6, 6 }
    local limit = 18
    ...
    local current_idx = (G.your_collection_page - 1) * limit
    
    for j = 1, #G.your_collection do
        for i = 1, collection_rows[j] do
            current_idx = current_idx + 1
            local center = consumable_pool[current_idx]
            if not center then break end
            -- Spawn the cards with the corrected smaller dimensions on page flip
            local card = Card(G.your_collection[j].T.x + G.your_collection[j].T.w/2, G.your_collection[j].T.y, abil_w, abil_h, G.P_CARDS.empty, center)
            card:start_materialize(nil, i>1 or j>1)
            G.your_collection[j]:emplace(card)
        end
    end
